...
0) cs2.exe      = главный файл игры, весит обычно 4-5 MB (скопируйте сюда и переименуйте в 'cs2')
   или cs2.bin   = если .ехе файл вашей игры весит меньше 1MB - скопируйте .bin файл вместо .ехе (вес файла должен быть 4-5 MB)\n
...
После копирования всех архивов, подлежащих распаковке - нажмите Enter...""",
            ...
            "Отсутствуют файлы в папке tools: {0}\nСкачайте или извлеките архив заново. Нажмите Enter, чтобы закрыть окно."]
...
After copying all files in this folder press Enter...""",
                  ...
                  "Following tools are missing: {0}\nDownload or unpack archive again. Press Enter to exit."]
...
def check_all_tools_intact():
    global dir_path, locale_to_use
    dir = dir_path + "/tools/"
    tools = ["convert.php", "Decat2.exe", "exkifint_v3.exe", "extract_text_to_xlsx.py"]
    missing_files = ""
    for tool in tools:
        if not os.path.exists(dir + tool):
            missing_files += tool + " "
...
def extract_everything():
    ...
    #next extracting text lines from .txt files into .xlsx files
    pass
